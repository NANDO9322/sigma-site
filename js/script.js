...
const getBrazilStatesAndCityes = () => {
    return [
        {
          "sigla": "AC",
          "nome": "Acre",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    estadoSelect.innerHTML += getBrazilStatesAndCityes().map( est => `<option value="${est.sigla}">${est.sigla}</option>` ).join("")
  
    estadoSelect.addEventListener("change", e => {
      const selected = e.target.value
  
      const dados = getBrazilStatesAndCityes().find( _ => _.sigla === selected )
  
      if( dados )
      {
        citySelect.innerHTML = dados.cidades.map( city => `<option value="${city}">${city}</option>` ).join("")
      }
      else {
